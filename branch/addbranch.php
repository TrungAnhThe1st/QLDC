...
if (isset($_POST['txtBrName'])) {
  ...
  if (isset($_POST['hdn']) && $_POST['hdn'] == '0') {
    $sql_branch = "INSERT INTO `tblbranch`(`branch_name`, `area_id`, `b_email`, `b_contact_no`, `b_address`,security_guard_mobile, secrataty_mobile,moderator_mobile,building_make_year,building_image,`b_status`,builder_company_name,builder_company_phone,builder_company_address,building_rule) 
  VALUES ('$_POST[txtBrName]',$_POST[ddlArea],'$_POST[txtBrEmail]','$_POST[txtBrConNo]','$_POST[txtareaAddress]','$_POST[security_guard_mobile]','$_POST[secrataty_mobile]','$_POST[moderator_mobile]','$_POST[building_make_year]','$image_url','$_POST[radioStatus]','$_POST[builder_company_name]','$_POST[builder_company_phone]','$_POST[builder_company_address]','$_POST[building_rule]')";
    mysqli_query($link, $sql_branch);
    mysqli_close($link);
    $url = WEB_URL . 'branch/branchlist.php?m=add';
    header("Location: $url");
  } else {
    $sql_branch = "UPDATE `tblbranch` SET `area_id` = $_POST[ddlArea], `branch_name`='" . $_POST['txtBrName'] . "',`b_email`='" . $_POST['txtBrEmail'] . "',`b_contact_no`='" . $_POST['txtBrConNo'] . "',`b_address`='" . $_POST['txtareaAddress'] . "',`security_guard_mobile`='" . $_POST['security_guard_mobile'] . "',`secrataty_mobile`='" . $_POST['secrataty_mobile'] . "',`moderator_mobile`='" . $_POST['moderator_mobile'] . "',`building_make_year`='" . $_POST['building_make_year'] . "',building_image='" . $image_url . "', `b_status` ='" . $_POST['radioStatus'] . "', `builder_company_name` ='" . $_POST['builder_company_name'] . "', `builder_company_phone` ='" . $_POST['builder_company_phone'] . "', `builder_company_address` ='" . $_POST['builder_company_address'] . "',`building_rule` ='" . $_POST['building_rule'] . "' WHERE branch_id = '" . $_GET['id'] . "'";
    mysqli_query($link, $sql_branch);
    mysqli_close($link);
        ...
        <form onSubmit="return validateMe();" action="<?php echo $form_url; ?>" method="post" enctype="multipart/form-data">
          <div class="box-body row">
            ...
            <div class="form-group col-md-6">
              ...
              <select name="building_make_year" id="building_make_year" class="form-control">
                ...
                while ($rows = mysqli_fetch_array($rs)) { ?>
                  <option <?php if ($building_make_year == $rows['y_id']) {
                            echo 'selected';
                          } ?> value="<?php echo $rows['y_id']; ?>"><?php echo $rows['xyear']; ?></option>
                <?php } ?>
              </select>
            </div>
            <div class="form-group col-md-6">
              <label for="ddlArea"><span style="color:red;">*</span><?php echo "Khu"; ?> :</label>
              <select name="ddlArea" id="ddlArea" class="form-control">
                <option value="">--<?php echo "Chọn khu"; ?>--</option>
                <?php
                $rs = mysqli_query($link, "SELECT * FROM tbl_area order by id ASC");
                while ($rows = mysqli_fetch_array($rs)) { ?>
                  <option <?php if ($area_id == $rows['id']) {
                            echo 'selected';
            ...
            <div class="form-group col-md-12">
              ...
              &nbsp;&nbsp;
              <input class="minimal" type="radio" name="radioStatus" id="radioStatus" <?php if ($b_status == '1') {
                                                                                        echo 'checked=checked';
                                                                                      } ?> value="1" />
              &nbsp;&nbsp;<span><?php echo $_data['text_9']; ?></span>&nbsp;&nbsp;
              <input class="minimal" type="radio" name="radioStatus" id="radioStatus" <?php if ($b_status == '0') {
                                                                                        echo 'checked=checked';
                                                                                      } ?> value="0" />
              &nbsp;&nbsp;<span><?php echo $_data['text_10']; ?></span>
            </div>
          ...
          <div class="box-footer">
            <div class="form-group pull-right">
              ...
              <a class="btn btn-warning" href="<?php echo WEB_URL; ?>branch/branchlist.php"><i class="fa fa-reply"></i> <?php echo $_data['back_text']; ?></a>
            </div>
          </div>
