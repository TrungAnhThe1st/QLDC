...
if (isset($_POST['ddlBillType'])) {
	$year_id = getYearId(date('Y', strtotime(str_replace('/', '-', $_POST['txtBillDate']))));
	$month_id = date('n', strtotime(str_replace('/', '-', $_POST['txtBillDate'])));
	if (isset($_POST['hdn']) && $_POST['hdn'] == '0') {
		$sql = "INSERT INTO tbl_add_bill(bill_type,bill_date,bill_month,bill_year,total_amount,deposit_bank_name,bill_details,branch_id) 
		values('$_POST[ddlBillType]','$_POST[txtBillDate]','$month_id','$year_id','$_POST[txtTotalAmount]','$_POST[txtDepositBankName]','$_POST[txtBillDetails]','" . $_SESSION['objLogin']['branch_id'] . "')";
		mysqli_query($link, $sql);
		//mysqli_close($link);
		$url = WEB_URL . 'bill/bill_list.php?m=add';
		header("Location: $url");
	} else {
		$sql = "UPDATE `tbl_add_bill` SET `bill_type`='" . $_POST['ddlBillType'] . "',`bill_date`='" . $_POST['txtBillDate'] . "',`bill_month`='" . $month_id . "',`bill_year`='" . $year_id . "',`total_amount`='" . $_POST['txtTotalAmount'] . "',`deposit_bank_name`='" . $_POST['txtDepositBankName'] . "',`bill_details`='" . $_POST['txtBillDetails'] . "' WHERE bill_id='" . $_GET['id'] . "'";
		mysqli_query($link, $sql);
		...
		<div class="col-md-12">
						...
						<div class="form-group col-md-6">
							...
							<select name="ddlBillType" id="ddlBillType" class="form-control">
								...
								while ($row_unit = mysqli_fetch_array($result_unit)) { ?>
									<option <?php if ($bill_type == $row_unit['bt_id']) {
												echo 'selected';
											} ?> value="<?php echo $row_unit['bt_id']; ?>"><?php echo $row_unit['bill_type']; ?></option>
								<?php } ?>
							</select>
						</div>
						<div class="form-group col-md-6">
            ...
            <select name="ddlBillMonth" id="ddlBillMonth" class="form-control">
              ...
              <?php
				...
				while ($row_unit = mysqli_fetch_array($result_unit)) { ?>
              <option <?php if ($bill_month == $row_unit['m_id']) {
							echo 'selected';
          ...
          <div class="form-group col-md-6">
            ...
            <select name="ddlBillYear" id="ddlBillYear" class="form-control">
              ...
              <?php
				...
				while ($row_unit = mysqli_fetch_array($result_unit)) { ?>
              <option <?php if ($bill_year == $row_unit['y_id']) {
							echo 'selected';
              ...
              <?php } ?>
					...
					<div class="box-footer">
						<div class="form-group pull-right">
							...
							<a class="btn btn-warning" href="<?php echo WEB_URL; ?>bill/bill_list.php"><i class="fa fa-reply"></i> <?php echo $_data['back_text']; ?></a>
						</div>
					</div>
